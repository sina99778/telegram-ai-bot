...
async def _execute_image_generation(
    ...
    if not content_check.allowed:
        ...
        await message.reply(t(lang, "abuse.image_content_blocked"), parse_mode="HTML")
        return
...
async def _execute_image_edit(
    ...
    lang = _lang(db_user)
    if not photo:
        await message.reply(t(lang, "image.edit_photo_required"), parse_mode="HTML")
        return
    if not prompt:
        prompt = "Retouch and improve this image"
...
@image_router.message(ActionStates.waiting_for_image_prompt, F.text, F.chat.type == "private")
async def handle_image_prompt_state(
    message: Message,
    state: FSMContext,
    db_user: User,
    image_orchestrator: ImageOrchestrator,
    ...
    await state.clear()
    await _execute_image_generation(message, message.text.strip(), db_user, image_orchestrator)


@image_router.message(ActionStates.waiting_for_image_edit, F.photo, F.chat.type == "private")
async def handle_image_edit_state(
    message: Message,
    state: FSMContext,
    db_user: User,
    image_orchestrator: ImageOrchestrator,
...
async def handle_image_command(message: Message, command: CommandObject, db_user: User, image_orchestrator: ImageOrchestrator):
    ...
    await _execute_image_generation(message, command.args.strip(), db_user, image_orchestrator)
